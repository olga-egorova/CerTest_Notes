...
We consider here a toy example of a 3-layer beam with 2 interfaces, clamped at one end, with no extra load -- just gravity.

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
knitr::opts_knit$set(root.dir = normalizePath("."))  # set the wd
```
...
The following code specifies the virtual environment where the computer model (pyComp) lives, and the paths to the python scripts producing outputs for (1) generating "artificial" data (e.g. using finer grid) and (2) simulations. The paths need to be changed(!)
...
Picking the "true" theta-s (choosing at random in the region $[-\pi/30, \pi/30]$), running the FE model and saving the displacements.
...
coords_ind = c(15, 40, 137, 518, 2234, 2472, 2856, 2886, 2952, 2978)
...
Running the simulations: choosing $\theta$-s as a space-filling design ($\mbox{mogp_emulator\$LatinHypercubeDesign}$ function iin Python) from a "feasible" region: $\pm \pi/30$.
...
The final dataset $\mbox{dt_simulation}$ contains $n_simulation$ rows of location coordinates, $\theta$-s and simulation outputs.
...
#sim_ind = sort(maximin.cand(n = n_simulations, Tmax = 2*n_simulations, Xcand = as.matrix(dt_sim_coords))$inds)

sim_ind = c(19, 122, 128, 237, 263, 296, 328, 364, 468, 500, 628, 736, 821, 837, 1182, 1192, 1276, 1396, 
            1432, 1450, 1456, 1480, 1506, 1542, 1548, 1587, 2110, 2146, 2156, 2202)
...
dt_data = read.table("out/dt_data.csv", header = TRUE, sep = ",")
...
The covariance function for the GP approximating $\delta(x)$ is $$ \mbox{Cov}(\mathbf{x}^{(i)}, \mathbf{x}^{(j)}) =   \frac{1}{\lambda_\delta}\left[\mbox{exp}\left(-\frac{1}{2}\left[\frac{d_{x_i, x_j}^2}{\phi_x} + \frac{d_{y_i, y_j}^2}{\phi_y} + \frac{d_{z_i, z_j}^2}{\phi_z}\right]\right) + \delta_{ij}\tau^2_{\delta} x_i^2 \right] $$

Fitting can take a couple of minutes -- that does depend on the number of data and simulation points.
```{r}
# run model in stan
fit = stan(file = "Non_iso_delta_kernel.stan",
           data = stan_data,
           iter = 4000,
           chains = 3)
...
stan_trace(fit, pars = c("mu", "tf", "cl2_eta_inv", "cl2_delta_inv", 
                         "lambda_eta", "lambda_delta", "tau2_delta", "lambda_e"))  #"mu", 

# summarize results
print(fit, pars = c("mu", "tf", "cl2_eta_inv", "cl2_delta_inv", 
                    "lambda_eta", "lambda_delta", "tau2_delta", "lambda_e"))
...
Another model: with the non-isotropic part in the kernel for $\eta$ as well, not just the discrepancy term $\delta$. The number of correlation parameters is reduced: leave one for locations and one for $\theta-$s.

```{r}
# run model in stan
fit1 = stan(file = "Non_iso_eta_delta.stan",
           data = stan_data,
           iter = 4000,
           chains = 3)

# plot traceplots, excluding warm-up
stan_trace(fit1, pars = c("mu", "tf", "cl2_eta_inv", "cl2_delta_inv", 
                         "lambda_eta", "lambda_delta", "tau2_eta","tau2_delta", "lambda_e"))  #"mu", 

# summarize results
print(fit1, pars = c("mu", "tf", "cl2_eta_inv", "cl2_delta_inv", 
                    "lambda_eta", "lambda_delta", "tau2_eta", "tau2_delta", "lambda_e"))

# posterior probability distribution of tf
stan_hist(fit1, pars = c("tf"))
stan_hist(fit1, pars = c("mu"))

```



Look at pairs and triplets of theta posteriors.
```{r}
df_of_draws1 = as.data.table(fit1)              
setnames(df_of_draws1, old = c("tf[1]", "tf[2]", "tf[3]"),
         new = c("tf1", "tf2", "tf3"))
head(df_of_draws1)
...
par(cex.axis = 0.5)
k1 <- kde2d(df_of_draws1$tf1, df_of_draws$tf2, n=100)
image(k1, xlab = expression(theta[1]), ylab = expression(theta[2]))
# Adjust binning (interpolate - can be computationally intensive for large datasets)
k2 <- kde2d(df_of_draws1$tf3, df_of_draws$tf2, n=100)
image(k2, xlab = expression(theta[3]), ylab = expression(theta[2]))

k3 <- kde2d(df_of_draws1$tf1, df_of_draws$tf3, n=100)
image(k3, xlab = expression(theta[1]), ylab = expression(theta[3]))
